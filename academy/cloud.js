(() => {
    ...
    shell.querySelectorAll("[data-auth]").forEach(button => button.onclick = async () => {
      ...
      try {
        if (button.dataset.auth === "signin") {
          ...
          if (fullName.length < 3) throw new Error("Enter the learner’s full name for the certificate record.");
  ...
  function showAuth() { authModal(); document.getElementById("academyAuthModal").hidden = false; }
  ...
  window.rouxAcademyCloud = { client, showAuth, pullProgress, pushLesson, loadFinalQuestions, submitFinal, get session(){ return session; } };
  window.addEventListener("roux-academy-progress", event => pushLesson(event.detail).catch(() => {}));
  client.auth.onAuthStateChange((_event, nextSession) => {
    session = nextSession;
    renderIdentity();
    if (session) setTimeout(() => pullProgress().catch(console.error), 0);
  });
  client.auth.getSession().then(({ data }) => { session = data.session; authModal(); renderIdentity(); if (session) pullProgress().catch(console.error); });
})();
