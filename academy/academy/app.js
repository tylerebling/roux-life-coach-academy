...
function renderDashboard(){
  ...
  dashboardText("studentFirstName",firstName);dashboardText("profileName",fullName);dashboardText("profileInitial",firstName.slice(0,1).toUpperCase());dashboardText("phaseKicker",`${phase.name.toUpperCase()} Â· PHASE ${phaseIndex+1} OF ${dashboardPhases.length}`);dashboardText("phaseDescription",phase.description);
  dashboardText("currentLessonNumber",`LESSON ${String(view.current.number).padStart(2,"0")} Â· ${phase.name.toUpperCase()}`);dashboardText("currentLessonTitle",view.current.title);dashboardText("currentLessonMeta",`${view.current.minutes} min Â· Next: ${view.resumeActivity==="review"?"Review completed lesson":`Complete ${view.resumeActivity}`}`);
  const activityLabels={presentation:"Presentation",workbook:"Workbook",practice:"Practice",quiz:"Quiz"};
  document.getElementById("activityRow").innerHTML=Object.entries(view.activities).map(([key,done])=>`<span class="${done?"done":""}"><i>${done?"âœ“":"â—‹"}</i>${activityLabels[key]}</span>`).join("");
  ...
  document.getElementById("milestoneTrack").innerHTML=dashboardPhases.map((item,index)=>{const phaseLessons=lessons.filter(lesson=>lesson.number>=item.range[0]&&lesson.number<=item.range[1]),done=phaseLessons.every(lesson=>state.completed.includes(lesson.number)),current=index===phaseIndex,status=done?"Complete":current?"Current":index<phaseIndex?"Available":"Locked";return `<div class="milestone ${done?"complete":current?"current":"locked"}"><i>${done?"âœ“":index+1}</i><b>${item.name}</b><span>${status}</span></div>`}).join("");
  document.getElementById("weeklyTasks").innerHTML=Object.entries(view.activities).filter(([,done])=>!done).slice(0,3).map(([key])=>`<div><span>â—‹</span><p><b>${activityLabels[key]}</b><small>Lesson ${view.current.number}</small></p></div>`).join("")||'<div><span>âœ“</span><p><b>Lesson complete</b><small>Continue to your next lesson</small></p></div>';
  const progressRows=Array.isArray(remote.progress)?remote.progress:[],scores=progressRows.map(row=>Number(row.practice_score)).filter(Number.isFinite);dashboardText("practiceScore",scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):"â€”");
