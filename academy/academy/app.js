...
const dashboardLogout=$("#dashboardLogout");if(dashboardLogout)dashboardLogout.addEventListener("click",async()=>{if(confirm("Sign out of the academy?")){await window.rouxAcademyCloud?.client.auth.signOut();location.replace("/");}});
const sidebarLogout=$("#sidebarLogout");if(sidebarLogout)sidebarLogout.addEventListener("click",async event=>{event.preventDefault();if(confirm("Sign out of the academy?")){await window.rouxAcademyCloud?.client.auth.signOut();location.replace("/");}});
