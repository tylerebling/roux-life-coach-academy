...
function renderMaterials(){
  ...
  grid.innerHTML=lessons.map(lesson=>{
    ...
    return `<article class="material-card ${unlocked?"":"locked"}"><span>${String(lesson.number).padStart(2,"0")}</span><div><p>${lesson.division}</p><h3>${lesson.title}</h3><small>${lesson.number===1?"Lesson presentation and completion activity":"Workbook · reflection · practice · 12-question quiz"}</small></div>${unlocked?`<a href="${destination}">${done?"Review materials":"Open materials"} <span aria-hidden="true">→</span></a>`:`<span class="material-lock">Unlock after Lesson ${lesson.number-1}</span>`}</article>`;
  }).join("");
}
...
function renderExam(){
  ...
  $("#answeredCount").textContent=`${Object.keys(state.answers).length} of ${questions.length} answered`; $("#scoreDisplay").textContent=state.examScore===null?"":`${state.examScore}% · ${state.examScore>=80?"Passed":"Retake required"}`;
}
...
function renderAll(){renderTabs();renderLessons();renderMaterials();renderProgress();populateCertificate();}
