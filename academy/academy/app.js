...
const dashboardPhaseIcons=[
  '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 27V12M16 18c-5 0-8-3-8-8 5 0 8 3 8 8Zm0-3c0-5 3-8 8-8 0 5-3 8-8 8ZM10 27h12"/></svg>',
  '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M20 26h-9v-5c-2-2-3-5-3-8a9 9 0 1 1 15 7v6h-3"/><path d="m16 9 1.2 2.4 2.8.4-2 2 .5 2.7-2.5-1.3-2.5 1.3.5-2.7-2-2 2.8-.4Z"/></svg>',
  '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="m4 26 9-13 5 7 3-4 7 10M16 16V5m0 0h8l-3 3 3 3h-8"/></svg>',
  '<svg viewBox="0 0 32 32" aria-hidden="true"><circle cx="11" cy="11" r="4"/><circle cx="22" cy="12" r="4"/><path d="M3 27v-4a8 8 0 0 1 16 0v4m0-8a7 7 0 0 1 10 6v2"/></svg>',
  '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 3v3m9-1-2 3m7 5-4 1M7 5l2 3M2 13l4 1m5 3c0-3 2-6 5-6s5 2 5 5c0 3-3 4-3 7h-5c0-2-2-3-2-6Zm2 10h5m-4 3h3"/></svg>',
  '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M9 5h14l3 5-3 16-7-4-7 4-3-16Z"/><circle cx="16" cy="13" r="5"/><path d="m13 13 2 2 4-5"/></svg>'
];
...
function renderDashboard(){
  ...
  document.getElementById("weeklyTasks").innerHTML=Object.entries(view.activities).filter(([,done])=>!done).slice(0,3).map(([key])=>`<div><span class="task-icon task-icon-${key}" aria-hidden="true"></span><p><b>${activityLabels[key]}</b><small>Lesson ${view.current.number}</small></p></div>`).join("")||'<div><span class="task-icon task-icon-complete" aria-hidden="true"></span><p><b>Lesson complete</b><small>Continue to your next lesson</small></p></div>';
